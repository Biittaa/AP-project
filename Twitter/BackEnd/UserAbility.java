...
public class UserAbility implements Serializable {
    ...
    //	public void timeLine(HashSet<Tweet> tweets) {
//		HashMap<LocalDate, Tweet> showTweets = new HashMap<>();
//		for (Tweet t : tweets) {
//			if (t.getLikes() > 10) {
//				showTweets.put(t.getTweetDate(), t);
//			}
//		}
//		for (User u : followers) {
//			for (Tweet t : u.getAbility().getTweets()) {
//				showTweets.put(t.getTweetDate(), t);
//			}
//		}
//		TreeMap<LocalDate, Tweet> sortedTweets = new TreeMap<>();
//		sortedTweets.putAll(showTweets);
//		for (Tweet t : sortedTweets.values()) {
//			t.show();
//		}
//	}
//
	/*public void block(User user) {
		...
		for(User p : followers) {
			if(p.getID().equals(user.getID())) {
				this.unfollow(users, userID, user);
				break;
			}
		}
		for(User p : followings) {
			if(p.getID().equals(user.getID())) {
				this.unfollow(users, user.getID(), blocker);
				break;
			}
		}
    ...
    public void unblock(ArrayList<User> users, String userID, User user) {
        User unblocker = null;
        User unblocked = user;
        for (User u : users) {
  ...
  /*  public boolean checkIfBlocked(User u) {
    ...
    /*public void block(User u) {
        for (User p : followers) {
            if (p.getID().equals(u.getID())) {
                u.getAbility().unFollow(Client.controller.getUser(), u);
                break;
            }
        }
        for (User p : followings) {
            if (p.getID().equals(u.getID())) {
                Client.controller.getUser().getAbility().unFollow(u, Client.controller.getUser());
                break;
            }
        }
        u.getAbility().getInBlackList().add(Client.controller.getUser());
        blackList.add(u);
        for (int i = 0; i < Enter.users.size(); i++) {
            if (Enter.users.get(i).getID().equals(u.getID())) {
                Enter.users.remove(Enter.users.get(i));
    ...
    public void unBlock(User u) {
        blackList.remove(u);
        u.getAbility().getInBlackList().remove(Client.controller.getUser());
        for (int i = 0; i < Enter.users.size(); i++) {
            if (Enter.users.get(i).getID().equals(u.getID())) {
                Enter.users.remove(Enter.users.get(i));
                Enter.users.add(u);
